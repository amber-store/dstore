...
| file | what it is |
|---|---|
| `packstore/` | a core packstore, the local content-addressed store. Every tree fetched or pushed lands here, deduplicated. Its single-owner lock is the working copy's lock: two commands cannot run on one working copy at once. |
